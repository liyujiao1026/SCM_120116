...
Estimate_SCM <- function(data.SCM, control_rate, pre_inv_rate, 
            ...
            w.hat <- data.frame("ID" = w.hat[,1], "w.hat" =  as.numeric(w.hat[,2]))
            row.names(w.hat) <- NULL            
            
            
            return(list("w.hat" =  w.hat, "gap_1" = gap_1, "gap_n" = gap_n ))
...
Rep_SCM <- function(
            ...
            if (weight_constant == TRUE) { 
                        ...
                        w.h <- cbind(1:rep_times,round( t(sapply(1:rep_times, Fun.weight)), 4))
                        weight.hat <- data.frame(w.h)
                        ...
                        Fun.weight <- function(x){result[[x]]$w.hat[,2]}
            ...
            colnames(weight.hat) <- c("Rep_Times", paste0("unit",1:controlNumber))
            
            
            return( list("gap_1" = gap_1, "gap_n" = gap_n, "weight.hat" = weight.hat,
