...
DataGeneration <- function(
            ...
            X_control.Func <- function(controlNumber, timeLength, tau, omega) {
                        ...
                        X.base <- matrix(  #arima.sim(list(order = c(1,0,0), ar = tau), n = timeLength)
                        ...
                        X.control <- apply(X.individual, 2, function(x){ omega * X.base + (1 - omega) * x })
            ...
            X_treat1 <- matrix(apply( X_control1, 1, function(x){ sum(x*weight.sim)} ) + rho_x * e_x1, ncol = 1)
            
            X_treat2 <- matrix(apply( X_control2, 1, function(x){ sum(x*weight.sim)} ) + rho_x * e_x2, ncol = 1)
            
            #  3. Outcome of control units
            Y_control <- beta1 * X_control1 + beta2 * X_control2 + rho_xy * e_xy
            
            #  4. Outcome of treated unit (CounterFactual and Factual one )
            Y_treat.CountFact <- matrix(apply(Y_control, 1, function(x){ sum(x*weight.sim)} ) + rho_y * e_y , ncol = 1)
...
plotFun <- function(gap_1.v, gap_n.v){
            ...
            y_lim <- c( 0 , max(c(density(gap_1.v)$y, density(gap_n.v)$y)) + 1)
            ...
            text(x = gap_1.Q[1], y = -0.2,  gap_1.Q[1], cex = 0.85, col = "blue")
            text(x = gap_1.Q[2], y = -0.2,  gap_1.Q[2], cex = 0.85, col = "blue")
            text(x = mean(gap_1.v), y = 0.25,  round(mean(gap_1.v),3), cex = 0.9, col = "blue")
            ...
            text(x = gap_n.Q[1], y = -0.2,  gap_n.Q[1], cex = 0.85, col = "blue")
            text(x = gap_n.Q[2], y = -0.2,  gap_n.Q[2], cex = 0.85, col = "blue")
            text(x = mean(gap_n.v), y = 0.25,  round(mean(gap_n.v),3), cex = 0.9, col = "blue")
...
settingMatrix <- as.matrix(readRDS("./setting_matrix.RDS"))
# test
# settingMatrix <- as.matrix(readRDS("./setting_matrix.RDS"))
